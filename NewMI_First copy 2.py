...
class MutualInformationAnalyzer:
    ...
    def conditional_mi_all(self, x: np.ndarray, y: np.ndarray, 
                          ...
                          bandwidth: float = None) -> Tuple[float, float]:
        """
        ...
        bandwidth : float, optional
            Custom bandwidth to use for KDE. If None, calculates automatically.
    ...
    def analyze_alpha_reliability(self, 
        ...
        if control_vars:
            ...
            results['cond_mi_all'], results['p_value_all'] = self.conditional_mi_all(x, y, control_dict, optimal_bandwidth)
            ...
            if threshold_vars:
                ...
                results['cond_mi_threshold'], results['p_value_threshold'] = self.conditional_mi_threshold(x, y, threshold_dict, optimal_bandwidth)
            ...
            if ndt_vars:
                ...
                results['cond_mi_ndt'], results['p_value_ndt'] = self.conditional_mi_ndt(x, y, ndt_dict, optimal_bandwidth)
    ...
    def plot_parameter_specific_retentions(self, results: Dict, feature: str = 'alpha_s1', 
        ...
        # Additionally, create a bar chart comparing threshold and NDT retention values
        plt.figure(figsize=(7, 5))
        retention_values = [results['retention_threshold'], results['retention_ndt']]
        bar_labels = ['Threshold Only', 'NDT Only']
        
        bars = plt.bar(bar_labels, retention_values, color=['#1f77b4', '#ff7f0e'])
    ...
    def conditional_mi_bandwidth_sensitivity(self, 
                                           feature: str = 'alpha_s1',
                                           target: str = 'alpha_s2',
                                           control_vars: List[str] = None,
                                           bandwidths: List[float] = None,
        ...
        """
        Analyze how conditional MI and retention percentages vary with different bandwidths.
        ...
        if control_vars is None or len(control_vars) == 0:
            print("Error: Control variables must be provided for conditional MI analysis.")
            return None
        ...
        # Create dictionaries of control variables
        all_controls = {var: self.data[var].values for var in control_vars}
        threshold_controls = {var: self.data[var].values for var in threshold_vars}
        ndt_controls = {var: self.data[var].values for var in ndt_vars}
        
        # Calculate MI for each bandwidth
        for h in bandwidths:
            print(f"\nAnalyzing bandwidth: {h:.4f}")
            
            # Calculate raw MI
            raw_mi, _ = self.shrinkage_mi(x, y, alpha=alpha, bandwidth=h)
            results['raw_mi'].append(raw_mi)
            
            # Calculate conditional MI for all controls
            if all_controls:
                print(f"Calculating conditional MI for all parameters...")
                cmi_all, p_all = self.conditional_mi_all(x, y, all_controls, bandwidth=h)
                results['cond_mi_all'].append(cmi_all)
                results['p_values_all'].append(p_all)
                results['retention_all'].append((cmi_all / raw_mi) * 100 if raw_mi > 0 else 0)
            
            # Calculate conditional MI for threshold only
            if threshold_controls:
                print(f"Calculating conditional MI for threshold parameters...")
                cmi_threshold, p_threshold = self.conditional_mi_threshold(x, y, threshold_controls, bandwidth=h)
                results['cond_mi_threshold'].append(cmi_threshold)
                results['p_values_threshold'].append(p_threshold)
                results['retention_threshold'].append((cmi_threshold / raw_mi) * 100 if raw_mi > 0 else 0)
            
            # Calculate conditional MI for NDT only
            if ndt_controls:
                print(f"Calculating conditional MI for NDT parameters...")
                cmi_ndt, p_ndt = self.conditional_mi_ndt(x, y, ndt_controls, bandwidth=h)
                results['cond_mi_ndt'].append(cmi_ndt)
                results['p_values_ndt'].append(p_ndt)
                results['retention_ndt'].append((cmi_ndt / raw_mi) * 100 if raw_mi > 0 else 0)
                
        # Plot results
        self._plot_retention_by_bandwidth(results, dataset_name)
        
        return results
    
    def _plot_retention_by_bandwidth(self, results: Dict, dataset_name: str = None):
        """
        Plot retention percentages across different bandwidths.
        
        Parameters:
        -----------
        results : Dict
            Results from conditional_mi_bandwidth_sensitivity
        dataset_name : str, optional
            Name of the dataset to include in the title
        """
        plt.figure(figsize=(12, 8))
        
        # Plot retention percentages
        if 'retention_threshold' in results and len(results['retention_threshold']) > 0:
            plt.plot(results['bandwidths'], results['retention_threshold'], 'o-', 
                    label='Threshold Only', color='#1f77b4')
            
        if 'retention_ndt' in results and len(results['retention_ndt']) > 0:
            plt.plot(results['bandwidths'], results['retention_ndt'], 's-', 
                    label='NDT Only', color='#ff7f0e')
            
        if 'retention_all' in results and len(results['retention_all']) > 0:
            plt.plot(results['bandwidths'], results['retention_all'], '^-', 
                    label='All Parameters', color='#2ca02c')
        ...
        dataset_info = f" - {dataset_name}" if dataset_name else ""
        plt.title(f'Information Retention vs Bandwidth{dataset_info}', fontsize=14)
        plt.legend(fontsize=10)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.show()
        
        # Plot conditional MI values
        plt.figure(figsize=(12, 8))
        
        # Plot raw MI
        plt.plot(results['bandwidths'], results['raw_mi'], 'k-', label='Raw MI')
        
        # Plot conditional MI values
        if 'cond_mi_threshold' in results and len(results['cond_mi_threshold']) > 0:
            plt.plot(results['bandwidths'], results['cond_mi_threshold'], 'o-', 
                    label='Threshold Only', color='#1f77b4')
            
        if 'cond_mi_ndt' in results and len(results['cond_mi_ndt']) > 0:
            plt.plot(results['bandwidths'], results['cond_mi_ndt'], 's-', 
                    label='NDT Only', color='#ff7f0e')
            
        if 'cond_mi_all' in results and len(results['cond_mi_all']) > 0:
            plt.plot(results['bandwidths'], results['cond_mi_all'], '^-', 
                    label='All Parameters', color='#2ca02c')
        
        plt.xlabel('Bandwidth', fontsize=12)
        plt.ylabel('Mutual Information (bits)', fontsize=12)
        plt.title(f'Conditional MI vs Bandwidth{dataset_info}', fontsize=14)
        plt.legend(fontsize=10)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.show()
        ...
        if 'p_values_threshold' in results and len(results['p_values_threshold']) > 0:
            plt.plot(results['bandwidths'], results['p_values_threshold'], 'o-', 
                    label='Threshold Only', color='#1f77b4')
            
        if 'p_values_ndt' in results and len(results['p_values_ndt']) > 0:
            plt.plot(results['bandwidths'], results['p_values_ndt'], 's-', 
                    label='NDT Only', color='#ff7f0e')
            
        if 'p_values_all' in results and len(results['p_values_all']) > 0:
            plt.plot(results['bandwidths'], results['p_values_all'], '^-', 
                    label='All Parameters', color='#2ca02c')
        
        plt.axhline(y=0.05, color='red', linestyle='--', alpha=0.5, label='p=0.05')
        plt.xlabel('Bandwidth', fontsize=12)
        plt.ylabel('P-value', fontsize=12)
        plt.title(f'Significance vs Bandwidth{dataset_info}', fontsize=14)
        plt.legend(fontsize=10)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.show()
        
    def individual_parameter_sensitivity(self, 
                                       feature: str = 'alpha_s1',
                                       target: str = 'alpha_s2', 
                                       control_vars: List[str] = None,
                                       bandwidths: List[float] = None,
                                       ...
                                       dataset_name: str = None) -> Dict:
        """
        Analyze how conditional MI varies when controlling for each individual parameter across different bandwidths.
        ...
        target : str
            Column name for alpha from session 2
        control_vars : List[str]
            List of control variables to analyze individually
        bandwidths : List[float], optional
            List of bandwidths to test. If None, uses default range.
        alpha : float
            Shrinkage parameter (0 to 1)
        ...
        --------
        Dict
            Dictionary containing results for each parameter across bandwidths
        """
        if control_vars is None or len(control_vars) == 0:
            print("Error: Control variables must be provided for individual parameter analysis.")
            return None
        ...
        results = {
            'bandwidths': bandwidths,
            'raw_mi': [],
            'individual_cmi': {var: [] for var in control_vars},
            'individual_pval': {var: [] for var in control_vars},
            'individual_retention': {var: [] for var in control_vars}
        }
        ...
        for h in bandwidths:
            ...
            # Calculate conditional MI for each individual control variable
            for var in control_vars:
                print(f"Calculating conditional MI for {var}...")
                z = self.data[var].values
                
                # Create a dictionary with a single control variable
                control_dict = {var: z}
                
                # Use the conditional_mi method to calculate conditional MI
                # We'll create a simplified version for a single control variable
                kd_x = KernelDensity(bandwidth=h).fit(x.reshape(-1, 1))
                kd_y = KernelDensity(bandwidth=h).fit(y.reshape(-1, 1))
                kd_z = KernelDensity(bandwidth=h).fit(z.reshape(-1, 1))
                kd_xz = KernelDensity(bandwidth=h).fit(np.column_stack([x, z]))
                kd_yz = KernelDensity(bandwidth=h).fit(np.column_stack([y, z]))
                kd_xyz = KernelDensity(bandwidth=h).fit(np.column_stack([x, y, z]))
                ...
                for i in range(n_permutations):
                    y_perm = np.random.permutation(y)
                    kd_xyz_perm = KernelDensity(bandwidth=h).fit(np.column_stack([x, y_perm, z]))
                    log_pxyz_perm = kd_xyz_perm.score_samples(np.column_stack([x, y_perm, z]))
                    cmi_null[i] = np.mean(log_pxyz_perm + log_pz - log_pxz - log_pyz)
                ...
                # Store results
                results['individual_cmi'][var].append(cmi)
                results['individual_pval'][var].append(p_value)
                results['individual_retention'][var].append((cmi / raw_mi) * 100 if raw_mi > 0 else 0)
                
        # Plot results
        self._plot_individual_parameter_sensitivity(results, dataset_name)
        
        return results
    
    def _plot_individual_parameter_sensitivity(self, results: Dict, dataset_name: str = None):
        """
        Plot conditional MI for each individual control parameter across bandwidths.
        
        Parameters:
        -----------
        results : Dict
            Results from individual_parameter_sensitivity
        dataset_name : str, optional
            Name of the dataset to include in the title
        """
        ...
        colors = {
            ...
            'a_s1': '#1f77b4',       # blue
            'a_mean_s2': '#2ca02c',  # green
            'a_s2': '#2ca02c',       # green
            'ndt_mean_s1': '#ff7f0e', # orange
            'ndt_s1': '#ff7f0e',      # orange
            'ndt_mean_s2': '#d62728', # red
            'ndt_s2': '#d62728'       # red
        }
        
        # Line styles for session 1 vs session 2
        linestyles = {
            'a_mean_s1': '-',
            'a_s1': '-',
            'ndt_mean_s1': '-',
        ...
        # Plot 1: Conditional MI for each individual parameter
        plt.figure(figsize=(12, 8))
        
        # Plot raw MI for reference
        plt.plot(results['bandwidths'], results['raw_mi'], 'k-', label='Raw MI', linewidth=2)
        
        # Plot conditional MI for each parameter
        for var in results['individual_cmi'].keys():
            var_display = var
            if 'a_mean' in var or 'a_s' in var:
                var_display = f"Threshold ({var})"
            elif 'ndt' in var:
                var_display = f"NDT ({var})"
                
            plt.plot(results['bandwidths'], results['individual_cmi'][var], 
                   f"{markers.get(var, 'o')}-", label=var_display, 
                   color=colors.get(var, 'gray'), linestyle=linestyles.get(var, '-'))
        
        plt.xlabel('Bandwidth', fontsize=12)
        plt.ylabel('Mutual Information (bits)', fontsize=12)
        plt.title(f'Individual Parameter Conditional MI vs Bandwidth{dataset_info}', fontsize=14)
        plt.legend(fontsize=10)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.show()
        
        # Plot 2: Retention percentage for each individual parameter
        plt.figure(figsize=(12, 8))
        
        # Reference line at 100%
        plt.axhline(y=100, color='red', linestyle='--', alpha=0.5, label='100% Retention')
        
        # Plot retention for each parameter
        for var in results['individual_retention'].keys():
            var_display = var
            if 'a_mean' in var or 'a_s' in var:
                var_display = f"Threshold ({var})"
            elif 'ndt' in var:
            ...
            plt.plot(results['bandwidths'], results['individual_retention'][var], 
                   f"{markers.get(var, 'o')}-", label=var_display, 
                   color=colors.get(var, 'gray'), linestyle=linestyles.get(var, '-'))
        
        plt.xlabel('Bandwidth', fontsize=12)
        plt.ylabel('Information Retention (%)', fontsize=12)
        plt.title(f'Individual Parameter Retention vs Bandwidth{dataset_info}', fontsize=14)
        plt.legend(fontsize=10)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.show()
        
        # Plot 3: P-values for each individual parameter
        plt.figure(figsize=(12, 8))
        
        # Reference line at p=0.05
        plt.axhline(y=0.05, color='red', linestyle='--', alpha=0.5, label='p=0.05')
        
        # Plot p-values for each parameter
        for var in results['individual_pval'].keys():
            var_display = var
            if 'a_mean' in var or 'a_s' in var:
                var_display = f"Threshold ({var})"
            elif 'ndt' in var:
            ...
            plt.plot(results['bandwidths'], results['individual_pval'][var], 
                   f"{markers.get(var, 'o')}-", label=var_display, 
                   color=colors.get(var, 'gray'), linestyle=linestyles.get(var, '-'))
        
        plt.xlabel('Bandwidth', fontsize=12)
        plt.ylabel('P-value', fontsize=12)
        plt.title(f'Individual Parameter Significance vs Bandwidth{dataset_info}', fontsize=14)
        plt.legend(fontsize=10)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.show()
