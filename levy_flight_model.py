...
def visualize_levy_flight_model():
    ...
    n_trials = 50000    # Number of trials to simulate for distribution
    ...
    def simulate_trajectory(alpha, v, a, max_steps=5000):
        ...
        for step in range(max_steps):
            ...
            # Clip position to boundaries to ensure it doesn't exceed thresholds
            position[step+1] = np.clip(position[step+1], 0, a)
    ...
    for alpha in alpha_values:
        if correct_rts[alpha]:
            ...
            density = stats.gaussian_kde(correct_rts[alpha], bw_method=0.08)
        ...
        if error_rts[alpha]:
            # Use custom bandwidth for smoother KDE
            density = stats.gaussian_kde(error_rts[alpha], bw_method=0.08)
            xs = np.linspace(0, max_time, 2000)  # More points for smoother curve
            max_density = max(density(xs))
